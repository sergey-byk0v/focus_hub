...
async function getReasonSettings() {
...
chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
  ...
  try {
    const settings = await getReasonSettings();
    if (!settings.enabled) return;

    const url = new URL(details.url);
    if (!isTargetSite(url.hostname, settings.sites)) return;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'GET_REASON_SETTINGS') {
    (async () => {
      const settings = await getReasonSettings();
  ...
  if (message.type === 'SAVE_REASON_SETTINGS') {
    (async () => {
      await chrome.storage.local.set({ reasonSettings: message.settings });
      sendResponse({ success: true });
    })();
    return true;
  }
